...
- status (varchar, not null) — статус участия (например, "Зарегистрирован", "Завершил")
9. Review (Отзыв):
...
- id (int, PK, auto-increment) — уникальный идентификатор сертификата
- test_id (int, FK, not null) — тест, за успешное прохождение которого выдан сертификат
- user_id (int, FK, not null) — участник, получивший сертификат
- issue_date (timestamp, not null) — дата выдачи сертификата
- score (int, not null) — минимальный балл, при котором участник получает сертификат
- certificate_url (varchar) — URL для загрузки сертификата (если есть PDF-версия или другой файл)
...
- One-to-Many с Certificate (Сертификат) — один пользователь может получить множество сертификатов.
...
- One-to-Many с Certificate (Сертификат) — один тест может предоставлять множество сертификатов.
...
- Many-to-One с Test (Тест) — каждый сертификат выдается за один тест.
- Many-to-One с User (Пользователь) — каждый сертификат принадлежит одному пользователю.
